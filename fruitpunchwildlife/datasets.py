...
class DatasetLoader:
    ...
    def prepare_datasets(self, drop_na=True):
        ...
        print("Unify dataset")
        self.df = pd.concat([self.df_dict[dataset] for dataset in self.df_dict])
    ...
    def _load_dataset(self, drop_na):
        for dataset in self.config:
            ...
            if drop_na:
                ...
                self.config[dataset]['drop_na']=len(df[df.isnull().any(axis=1)])
            else:
    ...
    def _collect_categories(self, df_dict:dict):
        ...
        for dataset in df_dict:
            ...
            for name in df[['commonName','class_id']].value_counts().keys():
                list_names.append([name[0], name[1],df[['commonName','class_id']].value_counts()[name]])
...
#             print(dataset)
    ...
    def apply_new_index(self):
        ...
        new_index_dict = {row[0]:row[1] for index, row in  self.new_index.iterrows()}

        print("Creating new column: class_id_updated")
        self.df['class_id_updated'] = self.df.apply(lambda row: int(new_index_dict[row["commonName"]]), axis=1)
        
        print("Updating references in bbox.")
        print("Creating new column: yolo_bboxes_updated")
        self.df['yolo_bboxes_updated'] = self.df.apply(lambda row: self.update_yolo_bboxes(row), axis=1)
    ...
    def prepare_trapper_regions(self):
        self.df['deploymentID_region']=self.df.apply(lambda row: row['deploymentID'].replace('_','-'), axis=1)
        self.df['deploymentID_region']=self.df.apply(lambda row: row['deploymentID_region'].split('-')[0], axis=1)
        print('Created deploymentID_region')
        
        self.df['deploymentID_category']=self.df['deploymentID_region'].astype('category')
...
#             self.prepare_trapper_regions()
        ...
        df_balanced = self._balance_dataset(df_filtered, main_fields, max_samples)
...
#         pprint(min_count)
        ...
        for _, row in counts.iterrows():
            ...
            subset = df[(df['commonName'] == common_name) & (df['deploymentID_category'] == deployment_category)]
            if len(subset)<max_samples:
                balanced_subset=subset
            else:
                balanced_subset = subset.sample(n=max_samples, replace=True, random_state=42)
...
def split_balanced_dataset(df, stratify_fields=None, pickle_filename = None): 
    ...
    train_images, val_images, train_annotations, val_annotations = train_test_split(df['media_path'].to_list(), df['annotation_path'].to_list(), test_size = 0.2, random_state = 42, stratify=stratify)
...
def prepare_yolov58(split_dict, df):
    ...
    list_files = [('train.txt',split_dict['images']['train']),('val.txt',split_dict['images']['val']),('test.txt',split_dict['images']['test']),]
    ...
    data_yaml = {
        "names": classes,
        "nc": len(classes),
        "path": str(Path("./").resolve()), # this points to the base folder you find train.txt and the others...
        "train": "train.txt",
        "val": "val.txt",
        "test": "test.txt"
    }
    ...
    data_yaml_path = Path(f"{str(Path('./').resolve())}/yolo_info.yaml").resolve()
...
def prepare_yolonas(split_dict, target_folder):
    ...
    for category in ['images','labels']:
        ...
        for stage in ['test','train','val']:
            dict_folders[category][stage]=os.path.join(target_folder,category,stage)
            ...
            for filename in split_dict[category][stage]:
                shutil.copy(filename, os.path.join(dict_folders[category][stage],filename.split('/')[-1]))
